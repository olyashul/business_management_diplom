...
Назначение: модели данных для приложения products (каталог товаров).
Содержит:
- Supplier - модель поставщика (название, контактное лицо, телефон, email, адрес, ИНН);
- Category - модель категории товаров (название, slug, описание, изображение);
- Product - модель товара (артикул, название, категория, поставщик, цены, остатки, активность);
- ProductImage - модель изображений товара (связь с товаром, изображение, флаг основного, порядок);
- ProductAttribute - модель характеристик товаров (название, код, единица измерения);
- ProductAttributeValue - модель значений характеристик (связь с товаром и характеристикой);
- StockMovement - модель движения товаров на складе (тип движения, количество, предыдущий/новый остаток).
******************************************************************
"""
