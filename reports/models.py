"""
******************************************************************
Файл: reports/models.py
Назначение: модели данных для приложения reports (генерация отчетов).
Содержит:
- Report - модель отчета:
  * report_type - тип отчета (финансовый, по товарам, график работы, за день);
  * start_date, end_date - период отчета;
  * format - формат выгрузки (Excel);
  * category - фильтр по категории товаров (для отчета по товарам);
  * employee - фильтр по сотруднику (для отчета по графику работы);
  * generated_at - время генерации;
  * generated_by - пользователь, сгенерировавший отчет;
  * generate_data() - метод генерации данных отчета;
  * _generate_financial_data() - финансовый отчет (продажи, возвраты, выручка);
  * _generate_products_data() - отчет по товарам (количество продаж, выручка);
  * _generate_work_schedule_data() - отчет по графику работы (смены, часы);
  * _generate_daily_data() - отчет за день (продажи, сотрудники, товары).
******************************************************************
"""
